...
import { ApiError, api, assertValidUrl, friendlyMessage, shortUrlFor, type CreateLinkResponse } from "../lib/api";
  ...
  return (
    <div>
      <section aria-label="Shorten a link" className="pt-10 sm:pt-14">
        <p className="text-[15px] font-medium text-moss">A URL shortener that remembers every tap</p>
        ...
        {result && (
          <article aria-live="polite" className="ticket stamp-in mt-6 flex flex-col gap-4 rounded-2xl p-5 sm:flex-row sm:items-center">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 truncate text-[14px] text-pine/65" title={result.originalUrl}>
                {result.originalUrl}
              </p>
            </div>
            <div aria-hidden="true" className="ticket-perf hidden self-stretch sm:block" />
